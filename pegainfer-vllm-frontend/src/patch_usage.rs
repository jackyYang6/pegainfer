...
struct StreamingRequestCleanup {
    cached_tokens_by_request_id: CachedTokenUsageMap,
    request_id: Option<String>,
}

impl Drop for StreamingRequestCleanup {
    fn drop(&mut self) {
        let Some(request_id) = self.request_id.take() else {
            return;
        };
        let cached_tokens_by_request_id = self.cached_tokens_by_request_id.clone();
...
async fn remove_cached_tokens_pair(
    cached_tokens_by_request_id: &CachedTokenUsageMap,
    first: Option<&str>,
    second: Option<&str>,
) {
    let mut cached_tokens_by_request_id = cached_tokens_by_request_id.write().await;
    if let Some(first) = first {
        cached_tokens_by_request_id.remove(first);
    }
    if second != first {
        if let Some(second) = second {
            cached_tokens_by_request_id.remove(second);
        }
    }
}

async fn take_cached_tokens(
    cached_tokens_by_request_id: &CachedTokenUsageMap,
    request_id: &str,
    fallback_request_id: Option<&str>,
) -> u32 {
    let mut cached_tokens_by_request_id = cached_tokens_by_request_id.write().await;
    let cached_tokens = cached_tokens_by_request_id.remove(request_id);
    if fallback_request_id != Some(request_id) {
        if let Some(fallback_request_id) = fallback_request_id {
            return cached_tokens
                .or_else(|| cached_tokens_by_request_id.remove(fallback_request_id))
                .unwrap_or(0);
        }
    }
...
async fn forward_cached_token_usage_request(
    ...
    match vllm_router.oneshot(request).await {
        ...
        Ok(response) => patch_completion_usage(response, state, request_id_for_cleanup).await,
        Err(error) => {
            remove_cached_tokens(
                &state.cached_tokens_by_request_id,
                request_id_for_cleanup.as_deref(),
            )
            .await;
            (
...
fn request_id_from_completion_request(
    ...
    match parts.uri.path() {
        "/v1/chat/completions" => Some(format!("chatcmpl-{request_id}")),
        "/v1/completions" => Some(format!("cmpl-{request_id}")),
        _ => Some(request_id),
    }
}

async fn patch_completion_usage(
    response: Response,
    state: UsagePatchState,
    request_id_for_cleanup: Option<String>,
) -> Response {
    ...
    let response_request_id = value
        ...
        .and_then(serde_json::Value::as_str)
        .map(ToString::to_string);
    let cleanup_request_id = response_request_id
        .as_deref()
        .or(request_id_for_cleanup.as_deref());

    if !status.is_success() {
        remove_cached_tokens_pair(
            &state.cached_tokens_by_request_id,
    ...
    let Some(request_id) = cleanup_request_id else {
        return Response::from_parts(parts, Body::from(bytes));
    };
    let cached_tokens = take_cached_tokens(
        &state.cached_tokens_by_request_id,
        request_id,
        request_id_for_cleanup.as_deref(),
    )
...
async fn patch_streaming_completion_usage(
    response: Response,
    state: UsagePatchState,
    include_usage: bool,
    request_id_for_cleanup: Option<String>,
    ...
    let mut cleanup = StreamingRequestCleanup {
        cached_tokens_by_request_id: cached_tokens_by_request_id.clone(),
...
mod tests {
    ...
    async fn read_next_stream_chunk(stream: &mut BodyDataStream) -> Bytes {
        stream
            .next()
            .await
    ...
    async fn wait_until_cache_entry_removed(
        cached_tokens_by_request_id: &CachedTokenUsageMap,
        request_id: &str,
    ) {
        timeout(Duration::from_secs(1), async {
            loop {
                if !cached_tokens_by_request_id
                    .read()
                    .await
    ...
    #[tokio::test]
    async fn non_streaming_unparseable_response_cleans_known_request_id() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("cmpl-known".to_string(), 7),
            ("cmpl-other".to_string(), 9),
        ])));
        let vllm_router = Router::new().route(
            "/v1/completions",
            post(|| async {
        ...
        let map = cached_tokens_by_request_id.read().await;
        assert!(!map.contains_key("cmpl-known"));
        assert_eq!(map.get("cmpl-other"), Some(&9));
    }

    #[tokio::test]
    async fn non_streaming_missing_response_id_cleans_known_request_id_without_patching() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("cmpl-known".to_string(), 7),
            ("cmpl-other".to_string(), 9),
        ])));
    ...
    #[tokio::test]
    async fn non_streaming_unparseable_response_cleans_header_request_id_without_body_id() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("chatcmpl-header-known".to_string(), 7),
            ("chatcmpl-other".to_string(), 9),
        ])));
        let vllm_router = Router::new().route(
            "/v1/chat/completions",
            post(|| async {
        ...
        let request = Request::builder()
            .method("POST")
            .uri("/v1/chat/completions")
            .header("X-Request-Id", "header-known")
            .body(Body::from(
                serde_json::json!({
                    "model": "model",
                    "messages": [{"role": "user", "content": "hello"}]
        ...
        assert_eq!(&bytes[..], b"not json");
        let map = cached_tokens_by_request_id.read().await;
        assert!(!map.contains_key("chatcmpl-header-known"));
        assert_eq!(map.get("chatcmpl-other"), Some(&9));
    }

    #[tokio::test]
    async fn non_streaming_non_success_response_id_cleans_response_and_fallback_ids() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("cmpl-error".to_string(), 7),
            ("cmpl-known".to_string(), 8),
            ("cmpl-other".to_string(), 9),
        ])));
        let vllm_router = Router::new().route(
            "/v1/completions",
            post(|| async {
                Response::builder()
                    ...
                    .body(Body::from(r#"{"id":"cmpl-error","error":"bad request"}"#))
                    .expect("response")
            }),
        );
        let router = cached_token_usage_routes(vllm_router, cached_tokens_by_request_id.clone());
        ...
        let bytes = to_bytes(response.into_body(), COMPLETION_USAGE_PATCH_BODY_LIMIT)
            .await
            .expect("read body");
        assert_eq!(&bytes[..], br#"{"id":"cmpl-error","error":"bad request"}"#);
        let map = cached_tokens_by_request_id.read().await;
        assert!(!map.contains_key("cmpl-error"));
        assert!(!map.contains_key("cmpl-known"));
        assert_eq!(map.get("cmpl-other"), Some(&9));
    }

    #[tokio::test]
    async fn non_streaming_non_success_without_response_id_cleans_fallback_and_preserves_response()
    {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("chatcmpl-header-known".to_string(), 7),
            ("chatcmpl-other".to_string(), 9),
        ...
        let vllm_router = Router::new().route(
            ...
            post(|| async {
                Response::builder()
                    ...
                    .body(Body::from(r#"{"error":"temporarily unavailable"}"#))
                    .expect("response")
            }),
        );
        let router = cached_token_usage_routes(vllm_router, cached_tokens_by_request_id.clone());
        ...
        let bytes = to_bytes(response.into_body(), COMPLETION_USAGE_PATCH_BODY_LIMIT)
            .await
            .expect("read body");
        assert_eq!(&bytes[..], br#"{"error":"temporarily unavailable"}"#);
        let map = cached_tokens_by_request_id.read().await;
        assert!(!map.contains_key("chatcmpl-header-known"));
        assert_eq!(map.get("chatcmpl-other"), Some(&9));
    }
    ...
    #[tokio::test]
    async fn streaming_non_success_cleans_fallback_request_id() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("chatcmpl-header-known".to_string(), 5),
            ("chatcmpl-other".to_string(), 9),
        ])));
        let vllm_router = Router::new().route(
            "/v1/chat/completions",
    ...
    #[tokio::test]
    async fn streaming_body_drop_before_first_id_cleans_fallback_request_id() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([
            ("chatcmpl-header-known".to_string(), 5),
            ("chatcmpl-other".to_string(), 9),
        ])));
        let (_chunk_tx, chunk_rx) =
            mpsc::unbounded_channel::<Result<Bytes, std::convert::Infallible>>();
        let chunk_rx = Arc::new(std::sync::Mutex::new(Some(chunk_rx)));
        let vllm_router = Router::new().route(
            "/v1/chat/completions",
            post(move || {
        ...
        wait_until_cache_entry_removed(&cached_tokens_by_request_id, "chatcmpl-header-known").await;
        assert_eq!(
            cached_tokens_by_request_id
                .read()
                .await
    ...
    #[tokio::test]
    async fn streaming_body_drop_cleans_cached_tokens_for_seen_request_id() {
        let cached_tokens_by_request_id = Arc::new(RwLock::new(HashMap::from([(
            "chatcmpl-drop".to_string(),
            5,
        )])));
        ...
        chunk_tx
            .send(Ok(Bytes::from(
                "data: {\"id\":\"chatcmpl-drop\",\"object\":\"chat.completion.chunk\",\"usage\":null}\n",
            )))
            .expect("send first chunk");
        let _ = read_next_stream_chunk(&mut stream).await;

        drop(stream);

        wait_until_cache_entry_removed(&cached_tokens_by_request_id, "chatcmpl-drop").await;
    }
}
